...
router.get('/duplicates', async (req, res) => {
  res.json(await repo.findDuplicateClientClusters());
});
...
router.post('/merge', requireAdmin, async (req, res) => {
  ...
  const winner = await dbGet('SELECT * FROM clients WHERE client_id = ?', [winner_id]);
...
router.post('/duplicates/ignore', requireAdmin, async (req, res) => {
  ...
  await repo.ignoreDuplicateCluster('client', member_ids);
  res.json({ ok: true });
});
...
router.post('/', requireAdmin, async (req, res) => {
  ...
  await dbRun('INSERT INTO clients (client_id, client_name, active, notes) VALUES (?, ?, ?, ?)', [
    client_id,
    client_name.trim(),
    active ? 1 : 0,
    notes,
...
router.put('/:id', requireAdmin, async (req, res) => {
  const existing = await dbGet('SELECT * FROM clients WHERE client_id = ?', [req.params.id]);
  ...
  await dbRun('UPDATE clients SET client_name = ?, active = ?, notes = ? WHERE client_id = ?', [
    client_name,
    active,
    notes,
    req.params.id,
...
router.delete('/:id', requireAdmin, async (req, res) => {
  ...
  const recordCerts = await dbAll(
    'SELECT certificate_path FROM employee_training_records WHERE client_id = ? AND certificate_path IS NOT NULL',
    [req.params.id]
  );
  ...
  const sessions = await dbAll('SELECT session_id, roster_pdf_path FROM training_sessions WHERE client_id = ?', [req.params.id]);
  for (const session of sessions) {
    if (session.roster_pdf_path && fs.existsSync(session.roster_pdf_path)) fs.unlink(session.roster_pdf_path, () => {});
    const attendeeCerts = await dbAll(
      'SELECT certificate_path FROM session_attendees WHERE session_id = ? AND certificate_path IS NOT NULL',
      [session.session_id]
    );
