...
router.get('/', (req, res) => {
  const row = db.prepare('SELECT * FROM feedback_form_settings WHERE id = ?').get('default');
  res.json(row);
});

router.put('/', requireAdmin, (req, res) => {
  const existing = db.prepare('SELECT * FROM feedback_form_settings WHERE id = ?').get('default');
  ...
  res.json(db.prepare('SELECT * FROM feedback_form_settings WHERE id = ?').get('default'));
});
